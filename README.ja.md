...
### スキル：`/run-teamwork`、`/benchmark-loop` & `/optimize-protocol`
...
#### `/run-teamwork` — ワンショットチームビルダー

プロジェクト説明を与えると、チームを設計し、エージェントを起動して構築します。分析なし、ループなし。

```bash
/run-teamwork "決済と管理パネル付きの EC サイトを構築"
```

1. プロンプトを分析して必要な技術ドメインとチーム規模を判定
2. リサーチファースト（調査→実装）のワークフローで PM システムプロンプトを生成
3. tmux（macOS/Linux）または Windows Terminal でエージェントを起動
4. すべてのタスクの完了を待機
5. 出力ディレクトリとファイル数を報告
